...
function addLink(d) {
   ...
   if (addLink.length > 0) {
       let editBtn = '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="edit">' + commonStr.view + '</button>'
       var opBtn = ''
       if (d.state==1){
            opBtn = '<button type="button" class="layui-btn layui-btn layui-btn-xs" lay-event="undeploy">'+ commonStr.undeploy +'</button>'
       } else {
            opBtn = '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="deploy">'+ commonStr.deploy +'</button>'
       }
       let delBtn  = '<button type="button" class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">'+commonStr.del+'</button>'
       return editBtn+'&nbsp;'+ opBtn + '&nbsp;' +delBtn;
   }
}
...
$('#addOver').click(function(){
    ...
    $('#addForm1 input, #addForm1 select, #addForm1 textarea, #addForm1 checkbox').each(function(){
          ...
          let inputV = $(this).val();
          if ('' == inputN || null == inputN || undefined == inputN || 'uploadFile' == inputN || 'keyFile' == inputN ) {
              //
          }else{
    ...
    formData.append("uploadFile",$('#addForm1 input[name="uploadFile"]')[0].files[0]);
    ...
    table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){ //查看
            layer.open({
                ...
                area: ['1000px', '600px'],
                title: 'List',
                content: ctx + '/admin/deployVer?id='+obj.data.id,
                anim: 'slideRight',
                shade: 0.6, // 遮罩透明度
                shadeClose: true, // 点击遮罩区域，关闭弹层
                maxmin: true, // 允许全屏最小化
        ...
        } else if(layEvent === 'undeploy'){

                    $.ajax({
                        url: ctx + '/api/deploy/undeploy',
                        type: 'post',
                        contentType: 'application/json',
                        data: JSON.stringify({id: obj.data.id }),
                        success: function (data, statusText) {
