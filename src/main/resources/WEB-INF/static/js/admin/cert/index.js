...
layui.use(function () {
  ...
  function addLink(d) {
    ...
    if (addLink.length > 0) {
      var renewBtn =
        '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="renew">' +
        commonStr.renew +
        '</button>'
      var issueBtn =
        '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="issue">' +
        commonStr.issue +
        '</button>'
      ...
      if (d.approach == enum_Approach.manual) {
        renewBtn =
          '<button type="button" class="layui-btn layui-btn-disabled layui-btn-xs" lay-event="renew" disabled>' +
          commonStr.renew +
          '</button>'
        issueBtn =
          '<button type="button" class="layui-btn layui-btn-disabled layui-btn-xs" lay-event="issue" disabled>' +
          commonStr.issue +
          '</button>'
      }
      let editBtn =
      ...
      let delBtn =
        ...
        '</button>'
      let dropBtn =
        '<button type="button" class="layui-btn layui-btn-danger layui-btn-xs" lay-event="drop">' +
        commonStr.clear +
        '</button>'
      let dumpBtn =
        '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="dump">' +
        commonStr.dump +
        '</button>'
      let imptBtn =
        '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="importData">' +
        commonStr.import +
        '</button>'
      let viewBtn =
        '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" lay-event="view">' +
        commonStr.view +
        '</button>'
    ...
    cols: [
      [
        ...
        { field: 'renew', title: 'renew' },
        {
          ...
          title: 'notBefore',
          width: 150,
          templet: "<div>{{!d.notBefore?'':layui.util.toDateString(d.notBefore, 'yyyy-MM-dd HH:mm:ss') }}</div>",
        },
        {
          field: 'notAfter',
          title: 'notAfter',
          width: 150,
          templet: "<div>{{!d.notAfter?'':layui.util.toDateString(d.notAfter, 'yyyy-MM-dd HH:mm:ss') }}</div>",
        },
