...
public class MonitorService implements LifecycleBean {
    ...
    public MonitorInfo getMonitorInfoOshi() {
        ...
        infoBean.setUsedMemory(FormatUtil.formatBytes(
                OshiUtil.getMemory().getTotal() - OshiUtil.getMemory().getAvailable()));
        ...
        infoBean.setMemRatio(NumberUtil.decimalFormat(
                "#.##%",
                NumberUtil.div(
                        OshiUtil.getMemory().getTotal() - OshiUtil.getMemory().getAvailable(),
    ...
    public List<DiskInfo> getDiskInfo() {
        ...
        for (OSFileStore fs : OshiUtil.getOs().getFileSystem().getFileStores()) {
            ...
            if (fs.getTotalSpace() != 0) {
                diskInfo.setPercent(NumberUtil.decimalFormat(
                        "#.##%", NumberUtil.div(fs.getTotalSpace() - fs.getUsableSpace(), fs.getTotalSpace())));
            } else {
