...
public class CertUtils {
    ...
    public static String[] extractSNI(X509Certificate x509Cert)
    ...
    private static Map<String, String> _extractSNI(X509Certificate x509Cert)
            throws InvalidNameException, CertificateParsingException {
    ...
    public static X509Holder genRSA(String subjectDN)
    ...
    private static X509Holder gen(
        ...
        if (issuerCA == null) {
            ...
            if (StringUtils.equalsIgnoreCase("EC", privKey.getAlgorithm())) {
                signer = new JcaContentSignerBuilder("SHA256with" + privKey.getAlgorithm() + "DSA")
                        .setProvider(BC)
                        .build(privKey);
            } else {
                signer = new JcaContentSignerBuilder("SHA256with" + privKey.getAlgorithm())
                        .setProvider(BC)
                        .build(privKey);
            }
            certBuilder = new JcaX509v3CertificateBuilder(
                    issuer, BigInteger.valueOf(System.currentTimeMillis()), notBefore, notAfter, issuer, publicKey);
        } else {
            // 构造 X.509 证书请求
            ...
            signer = new JcaContentSignerBuilder("SHA256with" + issuerCAKey.getAlgorithm())
                    .setProvider(BC)
                    .build(issuerCAKey);
        ...
        certBuilder.addExtension(
                Extension.basicConstraints, true, new BasicConstraints(issuerCA == null)); // CA flag is true
        ...
        //        PrivateKeyInfo pkInfo = PrivateKeyInfo.getInstance(converter.getPrivateKey((PrivateKey)
        // privKey).getEncoded());
