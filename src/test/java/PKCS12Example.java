...
public class PKCS12Example {
    ...
    private Certificate selfSign(KeyPair keyPair, Instant notBefore, Instant notAfter) {
        ...
        X509v3CertificateBuilder certificateBuilder = new X509v3CertificateBuilder(
                dnName, certSerialNumber, Date.from(notBefore), Date.from(notAfter), dnName, subjectPublicKeyInfo);
        try {
    ...
    private KeyStore createBasicKeystoreWithSelfSignedCert() {
        try {
            ...
            newKeystore.setKeyEntry(
                    keyAlias, domainKey.getPrivate(), keyPassword().toCharArray(), new Certificate[] {signedDomain});
            newKeystore.setKeyEntry(
                    accountKeyAlias, accountKey.getPrivate(), keyPassword().toCharArray(), new Certificate[] {
                        signedAccount
                    });
            return newKeystore;
